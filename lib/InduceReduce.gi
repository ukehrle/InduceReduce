...
##
InstallGlobalFunction( IndRedInit,
function(G)
local GR,i;
	...
	GR.orders:=List(GR.classreps, x->Order(x)); # element orders
        ...
        Info(InfoCTUnger, 2, "Induce/Restrict: orders of class reps: ",
             GR.orders);
	GR.perm:=Sortex(GR.orders,function(x,y) return y<x; end); # sort by decreasing order of representatives
	...
	GR.OrderPrimes:=List(GR.orders,x->Set(Factors(x))); # primes dividing the orders of class representatives
...
##
InstallGlobalFunction( IndRedGroupTools,
function()
local TR;
	TR:=rec();
	...
	## find the position of the conjugacy class containing g in GR.classes , ord is the order of g
	TR.FindClass:= function(GR,h,ord)
	local j;
		for j in [GR.ordersPos[ord]..GR.k] do
			if not GR.orders[j]=ord then break; fi;	
			if h=GR.classreps[j] then # first check if h actually equals one of the class representatives
				return j;
			fi;
		od;
		for j in [GR.ordersPos[ord]..GR.k] do
	...
	## conjugacy class representatives of cyclic group corresponding to the ordering of columns in Vandermonde
...
##
InstallGlobalFunction( IndRedReduceTools,
function()
local RedTR,ip, ipSparse;
	...
	ipSparse:= function(GR,x,y)
		return Sum(GR.Elementary.FusedClasses, i->x[i]*ComplexConjugate(y[i])*GR.ccsizes[i])/GR.n;
	end;
...
##
InstallGlobalFunction( IndRedInitElementary,
function(GR,TR)
	...
	if GR.Elementary.isCyclic then
		...
		GR.Elementary.classfusion:=powermap; # class fusion equals power map for cyclic group
		...
		GR.Elementary.ctblP:=CharacterTable(GR.Elementary.P); # character table of p-group
		GR.Elementary.classrepsP:=List(ConjugacyClasses(GR.Elementary.ctblP),x->Representative(x));
		...
		GR.Elementary.classrepsZ:=TR.ClassesCyclic(GR); # class representatives cyclic group
		GR.Elementary.kZ:=GR.orders[GR.IndexCyc]; # number of classes cyclic group
		if not IsBound(GR.powermaps[GR.IndexCyc]) then # if necessary, compute powermap
			TR.PowMap(GR,GR.IndexCyc);
		fi;
		powermap:=GR.powermaps[GR.IndexCyc];
		GR.Elementary.k:=GR.Elementary.kP*GR.Elementary.kZ; # number of classes elementary group
		...
		GR.Elementary.classfusion:=TR.ClassFusion(GR);
			# compute the fusion of conjugacy classes of the elementary group to conjugacy classes of G
		GR.Elementary.XP:=Irr(GR.Elementary.ctblP); # irreducible characters of the p-group
	...
	for i in GR.Elementary.FusedClasses do # eliminate some elementary subgroups:
		...
		if GR.Elementary.isCyclic then # some elementary groups contained in the cyclic group
	...
	if not GR.Elementary.isCyclic then
		for i in Set(powermap) do 
			# elementary subgroups, where the cyclic part is generated by a power of GR.Elementary.z
			# and the p-groups coincide
			if TR.pPart(GR.n/GR.ccsizes[i],GR.Elementary.p)=TR.pPart(GR.n/GR.ccsizes[GR.IndexCyc],GR.Elementary.p) and
	...
	for i in [0..GR.orders[GR.IndexCyc]-1] do   # derive the powermaps of powers of GR.Elementary.z
		if not IsBound(GR.powermaps[powermap[i+1]]) then
			GR.powermaps[powermap[i+1]]:=[];
			for j in [0..GR.orders[powermap[i+1]]-1] do
				Add(GR.powermaps[powermap[i+1]],powermap[ i*j mod GR.orders[GR.IndexCyc] +1 ]);
			od;
		fi;
	od;
	return;
...
##
InstallGlobalFunction( IndRedInduceCyc,
function(GR)
local ords, inds;
	GR.Elementary:=rec();
...
##
InstallGlobalFunction( IndRedInduce,
function(GR)
local mat, i, j;
...
local TR, RedTR, H, ccsizesH, temp, it;
	...
	while Size(GR.Ir)<GR.k and not (GR.NumberOfPrimes=0 and GR.NumberOfCyclic=0) do
		...
		Info(InfoCTUnger, 1, "Induce/Restrict: Trying [|Z|, |P|, k(E)] = ",
			[ GR.orders[GR.IndexCyc], GR.Elementary.n/GR.orders[GR.IndexCyc], GR.Elementary.k ]);
...
##
InstallGlobalFunction( IndRedtinI,
function(GR)
local irr,i,perm;
	...
	od;
	irr:=[];
	for i in [1..GR.k] do # permute irreducible characters back to the order of classes in GR.C
